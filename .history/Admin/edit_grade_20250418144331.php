...
$id= $_GET['id'];
$stmt = $dbh->query("SELECT * FROM grades where id='$id'");
...
if (isset($_POST['btnedit'])) {
  ...
  $remark = mysqli_real_escape_string($conn, $_POST['txtremark']);
    ...
    $sql = "UPDATE grades SET grade=?, remarks=?, min_score=?, max_score =? where id=?";
    $stmt= $dbh->prepare($sql);
    $stmt->execute([$grade, $remark, $min_score, $max_score,$id]);
    if($stmt) {
    ...
    <div class="card-body">
        <div class="row">
           ...
           <label for="exampleInputEmail1">Grade</label>
               <input type="text" name="txtgrade" class="form-control" id="exampleInputEmail1" value="<?php echo $row_grade['grade']; ?>" required>
                </div>
                <div class="form-group">
           <label for="exampleInputEmail1">Remark</label>
               <input type="text" name="txtremark" class="form-control" id="exampleInputEmail1" value="<?php echo $row_grade['remarks']; ?>" required>
                </div>
                <div class="form-group">
           <label for="exampleInputEmail1">Min Score</label>
               <input type="number" name="txtmin_score" class="form-control" id="exampleInputEmail1" value="<?php echo $row_grade['min_score']; ?>" required>
                </div>
                <div class="form-group">
           <label for="exampleInputEmail1">Max Score</label>
               <input type="number" name="txtmax_score" class="form-control" id="exampleInputEmail1" value="<?php echo $row_grade['max_score']; ?>" required>
                </div>
            </div>
