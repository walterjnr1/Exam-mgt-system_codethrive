...
if (empty($_SESSION['user_id'])) {
  ...
  $id= $_GET['id'];
  $stmt = $dbh->query("SELECT st.id as id, u.name AS teacher, s.name AS subject, c.name AS class FROM subject_teacher st JOIN users u ON st.user_id = u.id JOIN subjects s ON st.subject_id = s.id
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    echo '<option value="' . $row_allocation['id'] . '">' . $row_allocation['teacher'] . '</option>';
    ...
    <div class="form-group">
      ...
      <select name="cmdsubject" class="form-control" id="exampleInputEmail1" required>
       <?php 
    // Assuming you have a database connection established
    $query = "SELECT * FROM subjects where school_id='$school_id' and role='Teacher'";
    $result = mysqli_query($conn, $query);
    echo '<option value="' . $row_allocation['id'] . '">' . $row_allocation['teacher'] . '</option>';
    while($row = mysqli_fetch_assoc($result)) {
        echo '<option value="' . $row['id'] . '">' . $row['name'] . '</option>';
