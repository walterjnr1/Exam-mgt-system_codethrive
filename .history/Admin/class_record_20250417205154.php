...
<div class="card">
              ...
              <div class="card-body">
                <table width="626" height="98" class="table table-bordered table-striped" id="example1">
                  ...
                  <tbody>
                  <?php 
        $data = $dbh->query("SELECT classes.*, users.name As teachr, FROM classes INNER JOIN users ON classes.user_id=users.id WHERE classes.school_id='$school_id'")->fetchAll();
        $cnt = 1;
        foreach ($data as $row) {
        ?>
         <tr>
