...
if (empty($_SESSION['user_id'])) {
  ...
  $id= $_GET['id'];
  $stmt = $dbh->query("SELECT s.id as sid,  st.id as st_id, u.name AS teacher,u.id AS uid, s.name AS subject, c.name AS class FROM subject_teacher st JOIN users u ON st.user_id = u.id JOIN subjects s ON st.subject_id = s.id
...
<div class="wrapper">
  ...
  <div class="content-wrapper">
    ...
    $result = mysqli_query($conn, $query);
    echo '<option value="' . $row_allocation['sid'] . '">' . $row_allocation['teacher'] . '</option>';
    while($row = mysqli_fetch_assoc($result)) {
        echo '<option value="' . $row['id'] . '">' . $row['name'] . '</option>';
    }
    ?>
