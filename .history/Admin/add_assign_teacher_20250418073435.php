...
<div class="card">
        ...
        s.session, tr.term, ct.created_at FROM class_teachers ct  JOIN classes c ON ct.class_id = c.id
        JOIN users t ON ct.teacher_id = t.id JOIN sessions s ON ct.session_id = s.id
        JOIN terms tr ON ct.term_id = tr.id where school_id='$school_id' ORDER BY ct.assigned_date DESC")->fetchAll();
        $cnt = 1;
        foreach ($data as $row) {
        ?>
         <tr>
