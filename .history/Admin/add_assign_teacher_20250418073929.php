...
<div class="card">
              ...
              <div class="card-body">
                <table width="626" height="98" class="table table-bordered table-striped" id="example1">
                  ...
                  <?php 
        $data = $dbh->query("
        SELECT ct.id, c.name AS class_name, c.section, u.name AS teacher_name FROM class_teachers ct
        JOIN classes c ON ct.class_id = c.id JOIN users u ON ct.user_id = u.id WHERE ct.school_id='$school_id'
        ORDER BY ct.id DESC
      ")->fetchAll();
