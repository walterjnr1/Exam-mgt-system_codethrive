...
if (empty($_SESSION['user_id'])) {
  ...
  $id= $_GET['id'];
  $stmt = $dbh->query("SELECT st.id, u.name AS teacher, s.name AS subject, c.name AS class FROM subject_teacher st JOIN users u ON st.user_id = u.id JOIN subjects s ON st.subject_id = s.id
  JOIN classes c ON st.class_id = c.id where id='$id'");
  $row_subject = $stmt->fetch();

  if (isset($_POST['btnedit'])) {
  // Sanitize inputs
  $subject = mysqli_real_escape_string($conn, $_POST['txtname']);
  $class = mysqli_real_escape_string($conn, $_POST['txtname']);
  $teacher = mysqli_real_escape_string($conn, $_POST['txtname']);
