      ...
      <?php } ?>       </td>
       <td>
        ...
        <i class="fa fa-eye" aria-hidden="true" title="View All Student Record"></i>
          ...
          <a href="#" data-toggle="modal" data-target="#assign-student-modal">
          <i class="fa fa-user-plus action-icon" aria-hidden="true" title="Assign Student to Class"></i>
        </a>
        </td>
        </tr>
          <?php $cnt++; } ?>
          ...
          <!-- Modal Form -->
<div id="assign-student-modal" class="modal fade" role="dialog">
      ...
      <div class="modal-body">
        <form id="assign-student-form">
          <div class="form-group">
            ...
            <input type="text" id="#student-name" class="form-control" name="cmdname" readonly>
          </div>
          <div class="form-group">
            <label for="class-id">Class:</label>
            <select id="cmdclass" name="cmdclass" class="form-control">
              <?php
              // Fetch classes from database
              $classes = mysqli_query($conn, "SELECT MIN(id) as id, name FROM classes WHERE school_id = '$school_id' GROUP BY name ORDER BY name desc");
...
<script>
// Get student ID from icon click
var studentId = <?php echo $row['id'];?>;

// Populate student name in modal form
$('#assign-student-modal').on('show.bs.modal', function(event) {
...
// Update student class on form submit
$('#assign-student-form').submit(function(event) {
  event.preventDefault();
  var classId = $('#class-id').val();
  $.ajax({
    type: 'POST',
    url: 'update_student_class.php',
    data: {id: studentId, class_id: classId},
    success: function(data) {
      console.log(data);
      // Close modal and refresh page
      $('#assign-student-modal').modal('hide');
      location.reload();
    }
  });
});
