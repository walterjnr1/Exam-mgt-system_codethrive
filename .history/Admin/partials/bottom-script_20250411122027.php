...
<script>
    ...
    const file = event.target.files[0];
    const logoImg = document.getElementById('signature-img');
...
<script>
    ...
    const regionSelect = document.getElementById("cmdregion");
    const districtSelect = document.getElementById("cmddistrict");
    ...
    function updateDistricts() {
      ...
      if (ghanaData[selectedRegion]) {
        ghanaData[selectedRegion].forEach(district => {
          ...
          option.value = cmddistrict;
          option.textContent = cmddistrict;
