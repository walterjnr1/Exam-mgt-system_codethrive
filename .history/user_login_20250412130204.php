...
if (isset($_POST['btnlogin'])) {
  ...
  $sql = "SELECT u.id, u.email, u.password, u.school_id, s.code AS school_code
            FROM users u
            LEFT JOIN schools s ON u.school_id = s.id
            WHERE u.email = ? LIMIT 1";  $result = mysqli_query($conn, $query);
  ...
  if ($user = mysqli_fetch_assoc($result)) {
      if (password_verify($password, $user['password'])) {
          ...
          header("Location: dashboard.php"); // or wherever the user should land
      } else {
